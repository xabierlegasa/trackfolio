...
class TaxReturnYearAuditController
{
    ...
    public function show(int $year, string $isin): JsonResponse
    {
        ...
        $isin = strtoupper(trim(rawurldecode($isin)));
        $len = strlen($isin);
        if ($isin === '' || $len < 10 || $len > 12 || ! ctype_alnum($isin)) {
            return response()->json([
        ...
        try {
            $payload = $this->fifoTaxYearReport->buildIsinAudit($user->id, $year, $isin);
